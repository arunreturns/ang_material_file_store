module.exports = function(app) {
	...
	var gridFS 					= GridFS(conn.db, mongoose.mongo);
	...
	*/
	...
	function addFilesToZip(files, foldername, zip, res, cb){
		for (var file in files) {
			var fileData = files[file];
			console.log("{uploadAlt} => Processing file : [%s]",fileData.filename);
			var filename = fileData.filename;
			var filecontents = fileData.filecontents;
	
			var fileList = filename.split("/");
			var dirName = foldername + '/';
			for (var i = 0; i < fileList.length; i++) {
				var pathOrDir = fileList[i];
				if (pathOrDir.indexOf(".") !== -1) {
					try {
						var zipDir = dirName;
						zip = zip.file(zipDir + pathOrDir, filecontents);
						console.log("{uploadAlt} => File [" + pathOrDir + "] written to server");
					}
					catch (e) {
						console.log("{uploadAlt} => Error", e);
						res.status(500).send("Error while writing [" + pathOrDir + "]");
						res.end();
					}
				}
				else {
	...
	/* 
		Endpoint for uploading files to the server in a alternate way ;) .
		METHOD: POST
		Params: zipname, createzip, foldername, filelist
	*/
	app.post('/uploadAlt', function(req, res) {
		console.log("{uploadAlt} => Inside Upload Alternate");
		var zip = new JSZip();
		
		// Params
		var foldername = req.body.foldername;
		var files = req.body.filelist;
		var userName = req.body.userName;
		
		addFilesToZip(files, foldername, zip, res, function(){
			var meta = {
				userName: userName
			};
			...
			gridFS.writeFile({
			    ...
			    metadata: meta,
			}, buffer, function(err, file) {
				if ( err ) {
					console.log(err);
				}
				console.log('{uploadAlt} => Wrote to GridFS file', file._id);
				console.log("{uploadAlt} => File is ", file);
			    res.status(200).send("File Uploaded successfully");
			    res.end();
			});
		});
